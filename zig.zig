...
const global_table = extern struct {
    ...
    module_depend: *const fn ([*]const u8, [*]const u8, c_int, c_int) callconv(.C) ?[*]void,
    ...
    // 252 - 255
    egg_snprintf: *const fn () callconv(.C) void, // we dont need egg_snprintf() in zig
    egg_vsnprintf: *const fn () callconv(.C) void, // we dont need egg_vsnprintf() in zig
